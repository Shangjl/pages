...
```js
npm run newPage --template=simple
...
// ex: npm run dev --page=client --mobile=true
--mobile指定转换移动端样式
```
...
// ex: npm run build --page=client --mobile=true
--mobile指定转换移动端样式
```
...
```
创建新页面
npm run newPage --template=`scripts/config.js中的模板名称`
ex: npm run newPage --template=simple:mobile

输入 页面文件夹名称:页面描述
ex: TabPreview:拉片预览页面

运行  添加--mobile启用移动端css样式转换
npm run dev --page=`页面文件夹` --mobile --baseUrl=`页面运行路径`
ex: npm run dev --page=TabPreview --mobile --baseUrl=/TabPreview

构建 添加--mobile启用移动端css样式转换
npm run build --page=`页面文件夹` --mobile --baseUrl=`页面运行路径`
ex: npm run build --page=TabPreview --mobile --baseUrl=/multiple-page/TabPreview
```
